...
class Solver {
    ...
    private class StmtProcessor implements StmtVisitor<Void> {
        ...
        public Void visit(Invoke stmt) {
            if (stmt.isStatic()) {
                ...
                callGraph.addEdge(new Edge<>(CallGraphs.getCallKind(stmt),stmt,method));
                pass(stmt,method);
                addReachable(method);
            }
            return null;
    ...
    private void pass(Invoke callSite, JMethod method) {
        ...
        if (callSite.getLValue() != null) { // 排除返回值为void的情况
            method.getIR().getReturnVars().forEach(ret -> addPFGEdge(pointerFlowGraph.getVarPtr(ret),pointerFlowGraph.getVarPtr(callSite.getLValue())));
        }
    }
